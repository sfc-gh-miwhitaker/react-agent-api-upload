...
function source_env_file() {
    if [[ ! -f "$ENV_FILE" ]]; then
        ...
        echo "  ./tools/mac/01_setup_keypair_auth.sh --account YOUR_ACCOUNT_ID"
        exit 1
    fi
...
if [ "$PORT_CONFLICT" = true ]; then
    echo ""
    echo -e "${YELLOW}Tip: Run './tools/mac/04_stop.sh' to stop existing services${NC}"
...
echo "  2. Check status: ./tools/mac/03_status.sh"
echo "  3. Stop services: ./tools/mac/04_stop.sh"
echo ""
echo "================================================================================"
