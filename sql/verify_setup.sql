 ...
 * 
 * Validates all objects created by deploy_all.sql exist and are properly
 * configured. Run this after deployment to catch issues early.
 ******************************************************************************/
...
SELECT * FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())) WHERE "name" = 'SFE_DOCUMENTS_STAGE';

CREATE OR REPLACE TEMP TABLE VERIFY_STAGE_INFO AS
SELECT *
FROM INFORMATION_SCHEMA.STAGES
WHERE STAGE_NAME = 'SFE_DOCUMENTS_STAGE';

SHOW STREAMS IN SCHEMA REACT_AGENT_STAGE;
CREATE OR REPLACE TEMP TABLE VERIFY_STREAM AS
SELECT "name" AS name FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())) WHERE "name" = 'SFE_DOCUMENTS_STREAM';
...
WHERE "name" = 'SFE_EXTRACT_TEXT_TASK';
...
SELECT * FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())) WHERE "name" = 'SFE_DOCUMENT_METADATA';
...
WITH results AS (
  ...
  SELECT 'Stage directory enabled',
         CASE WHEN EXISTS (
           ...
           WHERE UPPER(CAST(DIRECTORY_ENABLED AS STRING)) = 'TRUE'
              ...
              THEN '✅ PASS' ELSE '❌ FAIL' END,
         'DESCRIBE STAGE SFE_DOCUMENTS_STAGE' AS detail
  UNION ALL
  SELECT 'Stage is internal (Snowflake-managed SSE)',
         CASE WHEN EXISTS (
           ...
           WHERE COALESCE(STAGE_URL, '') = ''
              ...
              THEN '✅ PASS' ELSE '❌ FAIL' END,
         'SHOW STAGES LIKE ''SFE_DOCUMENTS_STAGE''' AS detail
  UNION ALL
  ...
  SELECT 'Task state = STARTED',
         ...
         'Run ALTER TASK SFE_EXTRACT_TEXT_TASK RESUME; if WARN' AS detail
  UNION ALL
  SELECT 'Processing procedure exists',
         CASE WHEN EXISTS (
           SELECT 1 FROM INFORMATION_SCHEMA.PROCEDURES
           WHERE PROCEDURE_NAME = 'SFE_PROCESS_DOCUMENTS'
  ...
  SELECT 'Stage READ/WRITE granted to role',
         CASE WHEN EXISTS (
           ...
           WHERE PRIVILEGE IN ('READ','WRITE')
             ...
             AND NAME LIKE '%SFE_DOCUMENTS_STAGE%'
         )
              THEN '✅ PASS' ELSE '❌ FAIL' END,
  ...
  SELECT 'Process procedure granted to role',
         CASE WHEN EXISTS (
           ...
           WHERE PRIVILEGE = 'USAGE'
             ...
             AND NAME ILIKE '%SFE_PROCESS_DOCUMENTS%'
         )
              THEN '✅ PASS' ELSE '⚠️ WARN' END,
         'GRANT USAGE ON PROCEDURE ...SFE_PROCESS_DOCUMENTS() TO ROLE ...' AS detail
)
...
UNION ALL
SELECT 'Stream has data', SYSTEM$STREAM_HAS_DATA('SFE_DOCUMENTS_STREAM')::STRING;
